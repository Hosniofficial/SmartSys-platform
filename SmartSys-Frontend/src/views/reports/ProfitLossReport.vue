<template>
  <div class="min-h-screen bg-[#f8fafc] p-4 lg:p-8 font-cairo text-slate-700 animate-fadeIn text-right" dir="rtl">
    ...
    <PageHeader
      ...
      description="تحليل القوائم المالية، هوامش الربح، والمقارنات الدورية"
      :branches="[]"
      :selectedBranch="null"
    >
      <template #controls>
        <button @click="fetchReport" :disabled="isLoading" class="px-5 py-2.5 rounded-xl text-xs font-black text-slate-600 hover:bg-slate-50 transition-all flex items-center gap-2 bg-white border border-slate-100 shadow-sm">
