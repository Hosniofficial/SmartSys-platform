<template>
  <div class="min-h-screen bg-[#f8fafc] p-4 lg:p-8 text-slate-700 animate-fadeIn text-right" dir="rtl">
    ...
    >
      <template #controls>
        <button @click="fetchSessions" :disabled="isLoading" class="px-6 py-2.5 rounded-xl text-xs font-black text-slate-600 hover:bg-slate-50 transition-all flex items-center gap-2 bg-white border border-slate-100 shadow-sm">
    ...
    <BaseModal :show="showTransactionDetails && !!selectedSession" @close="closeSessionDetails" maxWidth="4xl" variant="modern">
      <template #header>
        <div class="flex items-center gap-4">
          ...
          <div>
            <h3 class="text-xl font-black text-slate-800 leading-none">تفاصيل الجلسة المحاسبية #{{ selectedSession?.session.id }}</h3>
            <p class="text-[10px] text-slate-400 mt-2 font-bold uppercase tracking-widest font-mono">تاريخ الجلسة: {{ formatDateTime(selectedSession?.session.start_time) }}</p>
          </div>
        </div>
      </template>
      ...
      <div class="space-y-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div class="p-4 rounded-2xl bg-slate-50 border border-slate-100"><p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">الفرع</p><p class="text-xs text-slate-800 font-bold">{{ selectedSession?.session.branch_name || 'الفرع الرئيسي' }}</p></div>
          <div class="p-4 rounded-2xl bg-slate-50 border border-slate-100"><p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">الجهاز</p><p class="text-xs text-slate-800 font-bold">{{ selectedSession?.session.device_name || 'غير محدد' }}</p></div>
          <div class="p-4 rounded-2xl bg-slate-50 border border-slate-100"><p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">الكاشير</p><p class="text-xs text-slate-800 font-bold truncate">{{ selectedSession?.session.closed_by_name || 'غير محدد' }}</p></div>
          <div class="p-4 rounded-2xl bg-slate-900 border border-slate-800 text-white"><p class="text-[9px] font-black text-blue-400 uppercase tracking-widest mb-1">حالة الجلسة</p><p class="text-xs font-black uppercase">{{ selectedSession?.session.status === 'closed' ? 'مغلقة ومؤرشفة' : 'مفتوحة حالياً' }}</p></div>
        </div>

        <!-- Time Info -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div class="p-4 rounded-2xl bg-slate-50 border border-slate-100"><p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">تاريخ ووقت البداية</p><p class="text-xs font-bold text-slate-800">{{ formatDateTime(selectedSession?.session.start_time) }}</p></div>
          <div class="p-4 rounded-2xl bg-slate-50 border border-slate-100"><p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">تاريخ ووقت النهاية</p><p class="text-xs font-bold text-slate-800">{{ selectedSession?.session.end_time ? formatDateTime(selectedSession?.session.end_time) : 'مفتوحة' }}</p></div>
        </div>

        <!-- Financial Reconciliation Box -->
        <div class="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-8 space-y-6">
          <h4 class="text-xs font-black text-slate-900 uppercase tracking-widest flex items-center gap-2"><i class="fas fa-calculator text-blue-500"></i> ملخص التسوية المالية</h4>
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="space-y-4">
              <div class="flex justify-between items-center text-xs font-bold text-slate-500"><span>الرصيد الافتتاحي:</span><span class="text-slate-800 font-mono">{{ formatCurrency(selectedSession?.calculated.opening_balance) }}</span></div>
              <div class="flex justify-between items-center text-xs font-bold text-emerald-600"><span>إجمالي الإيرادات (+):</span><span class="font-mono">+{{ formatCurrency(selectedSession?.totals.cash_in) }}</span></div>
              <div class="flex justify-between items-center text-xs font-bold text-rose-500 border-b border-slate-50 pb-3"><span>إجمالي المصروفات (-):</span><span class="font-mono">-{{ formatCurrency(selectedSession?.totals.cash_out) }}</span></div>
            </div>
            <div class="bg-slate-50 p-6 rounded-[1.5rem] flex flex-col justify-center text-center">
              ...
              <p class="text-2xl font-black text-slate-900 font-mono tracking-tighter">{{ formatCurrency(selectedSession?.calculated.expected_cash) }}</p>
            </div>
            <div class="bg-blue-600 p-6 rounded-[1.5rem] text-white flex flex-col justify-center text-center shadow-xl shadow-blue-100">
              ...
              <p class="text-2xl font-black font-mono tracking-tighter leading-none">{{ formatCurrency(selectedSession?.session.closing_cash_amount) }}</p>
          ...
          <div v-if="selectedSession?.session.variance_reason" class="text-sm text-gray-600 bg-amber-50 border border-amber-100 p-4 rounded-2xl">
            <span class="font-black text-amber-700">سبب الفرق: </span>{{ selectedSession?.session.variance_reason }}
          </div>
        </div>

        <!-- Transactions Table -->
        <div class="space-y-4">
          <h4 class="text-xs font-black text-slate-900 uppercase tracking-widest px-2 flex items-center gap-2"><i class="fas fa-list-ul text-blue-500"></i> سجل حركات الوردية</h4>
          <div class="bg-white rounded-[2rem] border border-slate-100 overflow-hidden shadow-sm">
            <table class="w-full text-right text-xs">
              <thead><tr class="bg-slate-50/50 text-slate-500 font-black border-b border-slate-100 uppercase tracking-tighter"><th class="px-6 py-4">الوقت</th><th class="px-4 py-4">نوع الحركة</th><th class="px-4 py-4">المبلغ</th><th class="px-4 py-4">المرجع</th><th class="px-6 py-4">الملاحظات</th></tr></thead>
              <tbody class="divide-y divide-slate-50 font-bold">
                <tr v-for="tx in selectedSession?.transactions" :key="tx.id" class="hover:bg-slate-50/50 transition-all group">
                  <td class="px-6 py-4 text-slate-400 font-mono tracking-tighter whitespace-nowrap">{{ formatDateTime(tx.created_at) }}</td>
                  <td class="px-4 py-4"><span class="px-2.5 py-1 bg-slate-100 text-slate-600 rounded-lg text-[10px] font-black uppercase">{{ getTransactionTypeLabel(tx.type) }}</span></td>
                  <td class="px-4 py-4 font-black font-mono tracking-tighter text-sm whitespace-nowrap" :class="isExpense(tx.type) ? 'text-rose-600' : 'text-emerald-600'">{{ isExpense(tx.type) ? '-' : '+' }} {{ formatCurrency(tx.amount) }}</td>
                  <td class="px-4 py-4 text-slate-500 whitespace-nowrap">{{ tx.reference_type }} #{{ tx.reference_id || '--' }}</td>
                  <td class="px-6 py-4 text-slate-400 italic font-medium truncate max-w-[180px]">{{ tx.notes || '--' }}</td>
                </tr>
                <tr v-if="!selectedSession?.transactions?.length"><td colspan="5" class="py-12 text-center text-slate-300 font-black uppercase tracking-widest">لا توجد حركات مالية مسجلة لهذه الجلسة</td></tr>
              </tbody>
            </table>
          </div>
        </div>
      ...
      <template #footer>
        <button @click="closeSessionDetails" class="px-8 py-2.5 rounded-xl text-xs font-black text-slate-500 hover:bg-slate-50 transition-all">إغلاق التقرير</button>
        <button v-if="selectedSession?.session.status === 'closed'" @click="exportSession(selectedSession?.session)" :disabled="isExporting" class="px-10 py-2.5 rounded-xl bg-blue-600 text-white text-xs font-black shadow-lg shadow-blue-100 hover:bg-blue-700 transition-all active:scale-95 flex items-center gap-2">
