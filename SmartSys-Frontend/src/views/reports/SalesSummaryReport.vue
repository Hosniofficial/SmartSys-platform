<template>
  <div class="min-h-screen bg-[#f8fafc] p-4 lg:p-8 font-cairo text-slate-700 animate-fadeIn text-right" dir="rtl">
    ...
    <PageHeader
      ...
      description="متابعة أداء المبيعات، الربحية، وحركة النقدية في نقاط البيع"
      :branches="[]"
      :selectedBranch="null"
    >
      <template #controls>
        <button @click="viewYesterday" class="px-5 py-2.5 rounded-xl text-xs font-black text-slate-600 hover:bg-slate-50 transition-all bg-white border border-slate-100 shadow-sm">عرض يوم أمس</button>
        <button @click="fetchSalesSummary" :disabled="isLoading" class="w-11 h-11 bg-white text-slate-400 rounded-xl flex items-center justify-center hover:text-blue-600 transition-all active:scale-90 border border-slate-100 shadow-sm">
