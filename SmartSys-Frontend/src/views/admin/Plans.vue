<template>
  <div class="min-h-screen bg-[#f8fafc] p-4 lg:p-8 text-slate-700 animate-fadeIn text-right" dir="rtl">
    ...
    <!-- Add Plan Modal -->
    <BaseModal :show="showAddModal" @close="showAddModal = false" maxWidth="lg" variant="modern">
      <template #header>
        <div class="flex items-center gap-4">
          <div class="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-200 text-white"><i class="fas fa-plus-circle text-xl"></i></div>
          <div>
      ...
      <form @submit.prevent="addPlan" class="space-y-6" dir="rtl">
        <div class="grid grid-cols-2 gap-6">
          <div class="space-y-2"><label class="modal-label">كود الخطة <span class="text-rose-500">*</span></label><input v-model="newPlan.code" type="text" required class="form-input-modern font-black font-mono uppercase tracking-widest" placeholder="PREMIUM_X" pattern="[a-z0-9_]+" /></div>
          <div class="space-y-2"><label class="modal-label">اسم الباقة <span class="text-rose-500">*</span></label><input v-model="newPlan.name" type="text" required class="form-input-modern font-bold" placeholder="باقة الشركات" /></div>
          <div class="space-y-2"><label class="modal-label">السعر المستهدف <span class="text-rose-500">*</span></label>
            <div class="relative">
          ...
          <div class="space-y-2"><label class="modal-label">دورة الفوترة (أيام) <span class="text-rose-500">*</span></label><input v-model.number="newPlan.billing_cycle_days" type="number" required class="form-input-modern font-black text-lg" placeholder="30" /></div>
        </div>
        <div class="flex flex-col gap-4">
          ...
          <label class="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100 cursor-pointer transition-all hover:bg-white hover:border-blue-100 group">
            <div class="flex-grow"><h4 class="text-sm font-black text-slate-800 leading-none">تنشيط الخطة</h4><p class="text-[10px] text-slate-400 font-bold mt-1.5 uppercase leading-none">إتاحة الباقة للاستخدام فور الحفظ</p></div>
      ...
      <template #footer>
        <button type="button" @click="showAddModal = false" class="px-8 py-2.5 rounded-xl border-2 border-slate-100 font-black text-slate-400 hover:bg-slate-50 transition-all text-xs uppercase tracking-widest">إلغاء</button>
        <button @click="addPlan" :disabled="addLoading" class="px-12 py-2.5 bg-blue-600 text-white rounded-xl font-black text-xs shadow-xl shadow-blue-100 hover:bg-blue-700 active:scale-95 transition-all flex items-center gap-3">
