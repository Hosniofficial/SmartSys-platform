...
class SalesHandler extends BaseHandler
{
    ...
    /**
     * ── AUDIT SELF-REVIEW FIX 15-A (إغلاق كامل) ──────────────────────────────
     * كانت هذه الدالة نسخة مستقلة تُحسب فقط للعرض (dynamic_status) بينما
     * AccountingService::recalculateSaleStatus() تكتب للعمود المخزَّن sales.status
     * بمنطق منفصل — نسختان لنفس المنطق كانتا عرضة للانحراف عن بعضهما مستقبلاً.
     * الآن كلتاهما تُفوِّضان لنفس المصدر الوحيد: AccountingService::computeSaleStatus().
     */
    ...
    {
        return (new AccountingService($this->db))
            ->computeSaleStatus($grandTotal, $paidAmount, $returnAmount, $returnCredits, $hasDirectReturn);
    }
    ...
    public function addSalesPayment(Request $request, Response $response): Response
    {
        ...
        // ── AUDIT FIX #2 (CRITICAL) ─────────────────────────────────────────────
        // This endpoint previously had NO Idempotency-Key handling at all, unlike
        // the adjacent payDebt() endpoint in this same file which uses
        // IdempotencyService correctly. A retried POST (network timeout, double
        // tap from a cashier, automatic client retry) would reach
        // SalePaymentService::addSalePayment() a second time. That method's own
        // internal guard (added in AUDIT FIX #1) now blocks the duplicate
        // payments/paid_amount write when the underlying journal entry is
        // reused — but relying on that alone still re-runs the full request
        // (cashier session lookup, credit allocation recompute, etc.) on every
        // retry. Adding the same header-based cache used by payDebt() short-
        // circuits an exact retry before any of that work happens, and matches
        // the pattern already proven correct elsewhere in this file.
        $idemKey = trim($request->getHeaderLine('Idempotency-Key'));
        $idem    = null;
        if ($idemKey !== '') {
            $idem   = new \App\Services\IdempotencyService($this->db);
            $cached = $idem->check((int) $tenantId, $idemKey);
            if ($cached !== null) {
                $this->logger->info('addSalesPayment: returning cached idempotent response', [
                    'tenant_id'       => $tenantId,
                    'idempotency_key' => $idemKey,
                ]);
                return $this->jsonResponse($response, $cached, 200);
        ...
        ");
            ...
            $result = $svc->addSalePayment(
                ...
                // ── AUDIT SELF-REVIEW FIX 15-B: pass the SAME explicit
                // Idempotency-Key used at the HTTP layer (AUDIT FIX #2) down to
                // the JE-level dedup check, so retry-detection is keyed off the
                // client's actual retry intent instead of guessing from
                // coincidentally-similar business data (date/amount/method/user).
            ...
            // Cache idempotent response for retry scenarios (same pattern as payDebt())
            if ($idemKey !== '' && $idem !== null) {
                $paymentId = (int) ($result['payment_id'] ?? 0);
                $idem->store((int) $tenantId, $idemKey, $paymentId, $responseData);
            }
