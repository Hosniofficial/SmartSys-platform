...
class AdvancedReportsHandler extends BaseHandler
{
    ...
    public function getDailyPerformance(Request $request, Response $response): Response
    {
        try {
            ...
            // 'completed' كانت قيمة ميتة لا تُكتب أبدًا (Finding الأصلي #5).
            // بعد إثراء sales.status (paid/settled_by_credit/closed_by_return/
            // settled_mixed/returned/...)، هذا مقياس "إجمالي مبيعات اليوم" —
            // يجب أن يشمل كل صور التسديد الكامل (نقدًا أو ائتمانًا أو مزيجًا)
            // لأن جميعها معاملات بيع حقيقية ولّدت إيرادًا، واستُبعدت عمدًا 'returned'
            // فقط (بيع أُعيد بالكامل = لا قيمة صافية متبقية).
            ...
            // ── AUDIT SELF-REVIEW FIX 15-A (vocabulary rollout) ──────────────────
            // على عكس مقياس "إجمالي المبيعات" أعلاه، هذا مقياس "نقد فعلي في
            // الدرج" (مربوط بـ pm.kind='cash') — يبقى مقصورًا على 'paid' فقط
            // (تسديد نقدي كامل فعلي). لا يجوز توسيعه لـ settled_by_credit أو
            // closed_by_return، لأن تلك الحالات — بتعريفها — لم يدخل فيها أي
            // نقد فعلي للدرج (سُدِّدت بالكامل عبر رصيد مرتجع)؛ توسيعها هنا كان
            // سيُنتج قيمة "نقد بالدرج" مبالغًا فيها ولا تطابق النقد الفعلي.
            $stmt = $this->db->prepare("
            ...
            $sql = "
                (SELECT
                    ...
                    'sale' AS type
                 FROM sales
                 WHERE status IN ('paid', 'settled_by_credit', 'closed_by_return', 'settled_mixed')
        ...
        try {
            ...
            if ($mode === 'cogs') {
                ...
                if ($cogsFromAccounting !== null && $cogsFromAccounting > 0) {
                    ...
                    // ⚠️ إصلاح معماري: كان هذا المسار يحسب COGS بصيغة مستقلة ثالثة
                    // (sales_items.quantity * purchase_price — سعر شراء لحظي غير دقيق تاريخياً)
                    // مختلفة تماماً عن CostingService (المستخدم فعلياً عند ترحيل COGS الحقيقي
                    // في AccountingService::postCOGSForSale) وعن SalesAnalyticsHandler.
                    // أصبح هذا المسار يستخدم نفس محرك WAC الحقيقي (CostingService) بدل صيغته
                    // الخاصة — نفس الرقم الذي رُحِّل فعلياً في القيود المحاسبية للفترة نفسها.
                    $costing = new \App\Services\CostingService($this->db);
                    ...
                    $periodSales = $stmt->fetchAll(PDO::FETCH_ASSOC) ?: [];

                    $cogs = 0.0;
                    foreach ($periodSales as $saleRow) {
                        $cogs += $costing->computeCOGSForSale(
                            $tenantId,
                            (int) $saleRow['id'],
                            $saleRow['sale_date'] ?: null
                        );
                    }

                    // ⚠️ إصلاح إضافي: الاستعلام الأصلي لم يكن يُصفِّي return_type، فكان
                    // يخلط مرتجعات المشتريات (غير ذات صلة بـ COGS المبيعات) مع مرتجعات
                    // المبيعات عند طرح COGS المرتجع. أُضيف الفلتر AND r.return_type='sale'.
                    // كما استُبدلت صيغة (purchase_price وقت المرتجع) بنفس محرك WAC دفعة واحدة
                    // لكل المنتجات المرتجعة، بدل استعلام سعر منفصل لكل صنف.
                    $stmt = $this->db->prepare("
                    ...
                    if (!empty($returnedProducts)) {
                        $productIds = array_map(static fn ($r) => (int) $r['product_id'], $returnedProducts);
                        $wacMap = $costing->computeBatchWAC($tenantId, $productIds, $endDate);
                        foreach ($returnedProducts as $row) {
                            $pid = (int) $row['product_id'];
                            $wac = $wacMap[$pid] ?? null;
                            if ($wac === null) {
                                continue; // نفس سلوك CostingService عند غياب سجل مشتريات (لا fallback صامت لسعر تخميني)
                            }
                            $returnsCOGS += (float) $row['qty'] * $wac;
                        }
                    }

                    $cogsSource = 'costing_engine_wac';
                }
