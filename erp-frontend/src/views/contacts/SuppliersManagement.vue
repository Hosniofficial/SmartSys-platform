  ...
  <div class="min-h-screen bg-[#f8fafc] text-slate-700 animate-fadeIn">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h1 class="text-2xl font-black text-slate-900 leading-none tracking-tight">إدارة الموردين</h1>
          <p class="text-slate-500 text-sm mt-2 font-medium italic">إدارة سجل الموردين، الذمم الدائنة، وجدولة الدفعات</p>
        </div>
      </div>
      ...
      <div class="flex flex-wrap items-center gap-3 bg-white p-2 rounded-[1.5rem] border border-slate-100 shadow-sm">
        ...
        <button @click="openAddModal" class="h-11 px-6 rounded-xl bg-indigo-600 text-white text-xs font-black shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all active:scale-95 flex items-center gap-2">
          <i class="fas fa-plus"></i> إضافة مورد جديد
        </button>
      </div>
    </div>

    <!-- KPIs Overview (Indigo Theme for Suppliers) -->
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
      <div v-for="kpi in kpis" :key="kpi.title" class="kpi-card group border-l-4 border-l-indigo-500">
        <div class="flex items-center gap-4">
    ...
    <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-4 mb-8 flex flex-col md:flex-row md:items-center gap-4">
      <div class="relative flex-grow group">
        <input type="text" v-model="search" class="form-input-modern pr-11" placeholder="ابحث بـ: اسم المورد أو رقم الهاتف..." />
        <i class="fas fa-search absolute right-4 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-indigo-500 transition-colors"></i>
      </div>
      
      <div class="flex items-center gap-3 shrink-0">
        <div class="flex items-center bg-slate-50 p-1 rounded-xl border border-slate-100">
          <button @click="viewMode = 'cards'" :class="[viewMode === 'cards' ? 'bg-white text-indigo-600 shadow-sm font-black' : 'text-slate-400']" class="px-4 py-2 rounded-lg text-xs transition-all flex items-center gap-2">
            <i class="fas fa-th-large"></i> بطاقات
          </button>
          <button @click="viewMode = 'table'" :class="[viewMode === 'table' ? 'bg-white text-indigo-600 shadow-sm font-black' : 'text-slate-400']" class="px-4 py-2 rounded-lg text-xs transition-all flex items-center gap-2">
    ...
    <div v-else-if="!filteredContacts.length" class="py-24 text-center px-6">
      <div class="flex flex-col items-center opacity-20 text-slate-400">
        <i class="fas fa-truck-ramp-box text-6xl mb-4"></i>
        <p class="font-black text-lg uppercase tracking-widest">لا يوجد موردين مضافين حالياً</p>
      </div>
      <button @click="fetchContacts" class="mt-6 text-indigo-600 font-black text-xs hover:underline uppercase tracking-widest">إعادة تحميل البيانات</button>
    </div>
      ...
      <div v-for="contact in filteredContacts" :key="contact.id" class="contact-card-modern group">
        ...
        <div class="flex items-start justify-between mb-6">
          <div class="flex items-center gap-3">
            <div class="w-12 h-12 bg-slate-100 rounded-2xl flex items-center justify-center text-slate-400 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-500">
              <i class="fas fa-truck text-lg"></i>
            </div>
            <div class="overflow-hidden">
              <h3 class="font-black text-slate-800 leading-none truncate mb-1.5 group-hover:text-indigo-600 transition-colors">{{ contact.name }}</h3>
              <p class="text-[11px] font-bold text-slate-400 font-mono tracking-tighter">{{ contact.phone || 'بدون هاتف' }}</p>
            </div>
          </div>
          <button @click="openPaymentModal(contact)" class="w-8 h-8 bg-emerald-50 text-emerald-600 rounded-lg flex items-center justify-center hover:bg-emerald-600 hover:text-white transition-all shadow-sm" title="تسجيل دفعة">
            <i class="fas fa-hand-holding-dollar text-xs"></i>
          </button>
        </div>

        <!-- Card Middle: Balance -->
        <div class="bg-slate-50 rounded-2xl p-4 mb-6 border border-slate-100 group-hover:bg-white group-hover:border-indigo-100 transition-all">
          <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">الرصيد المستحق</p>
          <div class="flex items-baseline gap-1.5">
            <span :class="[contact.balance > 0 ? 'text-emerald-600' : 'text-rose-600', 'text-xl font-black tracking-tighter']">
              {{ formatPrice(Math.abs(contact.balance)) }}
            </span>
            <span class="text-[10px] font-bold text-slate-400 uppercase">{{ contact.balance > 0 ? 'دائن' : (contact.balance < 0 ? 'مدين' : 'مستقر') }}</span>
          </div>
        </div>
        ...
        <table class="w-full text-right text-sm">
          <thead>
            <tr class="bg-slate-50/50 text-slate-500 font-black border-b border-slate-50 uppercase tracking-tighter">
              <th class="px-8 py-5">المورد</th>
              <th class="px-4 py-5">رقم الهاتف</th>
              <th class="px-4 py-5 text-center">الرصيد الحسابي</th>
              <th class="px-8 py-5 text-center">الإجراءات</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-50">
            <tr v-for="contact in filteredContacts" :key="contact.id" class="hover:bg-indigo-50/30 transition-all group">
              <td class="px-8 py-4">
                <div class="flex flex-col">
                  <span class="font-black text-slate-800 leading-none group-hover:text-indigo-600 transition-colors">{{ contact.name }}</span>
                  <span class="text-[10px] font-bold text-slate-400 mt-1.5 uppercase font-mono tracking-tighter">REF: {{ contact.code || '---' }}</span>
                </div>
              </td>
              <td class="px-4 py-4 font-bold text-slate-500 font-mono">{{ contact.phone || '-' }}</td>
              <td class="px-4 py-4 text-center">
                <span :class="['px-3 py-1 rounded-xl text-[10px] font-black uppercase tracking-tighter', contact.balance > 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700']">
    ...
    <!-- Modals (Logic strictly preserved) -->
      ...
      <div v-if="showFormModal" class="modal-overlay">
        <div class="modal-content-modern animate-modalIn max-w-xl">
          <div class="px-8 py-6 border-b border-slate-50 bg-slate-50/50 flex items-center justify-between">
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 bg-indigo-600 text-white rounded-xl flex items-center justify-center shadow-lg shadow-indigo-100">
                <i :class="selectedContact?.id ? 'fas fa-edit' : 'fas fa-truck-fast'"></i>
              </div>
              <h3 class="text-lg font-black text-slate-800 leading-none">{{ selectedContact?.id ? 'تعديل بيانات المورد' : 'إضافة مورد جديد' }}</h3>
            </div>
          ...
          <form @submit.prevent="handleFormSubmit(selectedContact)" class="p-8 space-y-6">
            <div class="space-y-2">
              <label class="modal-label">اسم المورد / الشركة <span class="text-rose-500">*</span></label>
              <input type="text" v-model="selectedContact.name" class="form-input-modern font-bold" placeholder="أدخل اسم المورد" required />
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="space-y-2">
                <label class="modal-label">رقم التواصل</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="space-y-2">
                <label class="modal-label">الرقم الضريبي (VAT)</label>
                <input type="text" v-model="selectedContact.tax_number" class="form-input-modern font-bold" />
              </div>
              <div class="space-y-2">
                <label class="modal-label">سقف المديونية</label>
                <input type="number" v-model.number="selectedContact.credit_limit" class="form-input-modern font-black text-indigo-600" min="0" step="0.01" />
              </div>
            </div>
            <div class="space-y-2">
              <label class="modal-label">عنوان المورد</label>
              <textarea rows="2" v-model="selectedContact.address" class="w-full rounded-2xl border border-slate-200 p-4 text-sm font-bold bg-slate-50 focus:bg-white transition-all outline-none focus:ring-4 focus:ring-indigo-50"></textarea>
            </div>
            <div class="flex justify-end gap-3 pt-4">
              ...
              <button type="submit" class="px-8 py-2.5 rounded-xl bg-indigo-600 text-white font-black shadow-lg shadow-indigo-100 hover:bg-indigo-700 active:scale-95 transition-all">حفظ البيانات</button>
            </div>
          </form>
        </div>
      </div>
      ...
      <div v-if="showPaymentModal" class="modal-overlay">
        <div class="modal-content-modern animate-modalIn max-w-md">
          <div class="p-8 text-center border-b border-slate-50 bg-slate-50/50">
            ...
            <h3 class="text-xl font-black text-slate-800 leading-none">تسجيل دفعة للمورد</h3>
            <p class="text-[10px] text-slate-400 mt-2 font-bold uppercase tracking-widest">المورد: {{ selectedContact.name }}</p>
          </div>
          <form @submit.prevent="handlePaymentSubmit" class="p-8 space-y-5">
            <div class="space-y-2">
              <label class="modal-label text-center block">المبلغ المدفوع <span class="text-rose-500">*</span></label>
            ...
            <div class="space-y-2">
              <label class="modal-label">ملاحظات العملية</label>
            ...
            <div class="flex gap-4 pt-2">
              ...
              <button type="submit" @click="handlePaymentSubmit" class="flex-[2] py-3.5 rounded-2xl bg-emerald-600 text-white font-black shadow-xl shadow-emerald-100 hover:bg-emerald-700 transition-all active:scale-95">حفظ الدفعة</button>
            </div>
          </form>
        </div>
      </div>
...
const { branches, initializePreferences, currentBranchName } = useUserBranchPreference('selectedSupplierBranch');
...
const onBranchChange = () => {
  if (selectedBranch.value) {
    try {
      window.localStorage?.setItem('selectedSupplierBranch', selectedBranch.value);
    } catch {}
  }
  fetchContacts();
};
...
const fetchContacts = async () => {
  ...
  try {
    ...
    if (!isExempt.value) {
      ...
      if (!wid) { showToast('لم يتم تعيين مخزن للمستخدم', 'warning'); return; }
...
/* Modern Components */
.form-input-modern, .form-select-modern { @apply w-full h-11 bg-white border border-slate-200 rounded-2xl px-4 outline-none transition-all duration-300 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-50 shadow-sm font-bold text-sm; }
.modal-label { @apply block text-[11px] font-black text-slate-400 uppercase tracking-widest mb-1.5 px-1; }

/* Cards Styling */
.contact-card-modern { @apply relative bg-white rounded-[2rem] shadow-sm border border-slate-100 p-7 transition-all duration-500 hover:shadow-xl hover:border-indigo-200; }
