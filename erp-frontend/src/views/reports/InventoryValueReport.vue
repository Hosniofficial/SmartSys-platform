<template>
  <div class="min-h-screen bg-[#f8fafc] p-4 lg:p-8 font-cairo text-slate-700 animate-fadeIn text-right" dir="rtl">
    ...
    <PageHeader
      ...
      description="تحليل القيمة الرأسمالية للبضاعة المتوفرة وتوزيعها المالي"
      :branches="[]"
      :selectedBranch="null"
    >
      <template #controls>
        <div class="flex items-center gap-2">
          ...
          <select v-model="valuation" class="form-select-modern h-9 text-xs font-black w-32">
            <option value="sale">سعر البيع</option>
            <option value="cost">سعر التكلفة</option>
          </select>
        </div>
        <button @click="fetchInventoryValue" :disabled="isLoading" class="w-11 h-11 bg-white text-slate-400 rounded-xl flex items-center justify-center hover:text-blue-600 transition-all active:scale-90 border border-slate-100 shadow-sm">
